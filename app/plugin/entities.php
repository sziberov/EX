<?
	...
	class Object_ extends Entity {
		...
		public function _getUploads() {
			...
			$sql = "SELECT * FROM uploads AS u WHERE u.object_id = $this->id ORDER BY u.title ASC";
			$query = $connection->query($sql);
			$this->uploads_ = [];
		...
		public function areMutualFriends($user_id) {
			...
			$sql = "SELECT id
					FROM links AS l
					WHERE (l.from_id = $this->id AND l.to_id = $user_id OR l.from_id = $user_id AND l.to_id = $this->id) AND l.type_id = 2";
			$query = $connection->query($sql);

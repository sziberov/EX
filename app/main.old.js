$(() => {
	...
	window.Dictionary = class {
		...
		static strings = {
			ru: {
				...
				string_groups_invites_count: 'Приглашений в группы',
			...
			en: {
				...
				string_groups_invites_count: 'Groups invites',
		...
		static getReplacementBlackzones(model) {
			...
			for(let cm of childModules) {
				let cmOuter = cm.outerHTML,
					...
					cmLeft = modelInner.indexOf(cmOuter, result.at(-1)?.[1])+cmOuter.substring(0, cmOuter.length-('</'+cm.nodeName+'>').length).lastIndexOf(cmInner)-1,
	...
	*/
	...
	window.toggleList = function(select) {
		for(let v of select.options) {
			$('.'+v.value).removeClass('current');
		}

		$('.'+select.value).addClass('current');
	}

	window.synchronizeSelect = function(e) {
		let select = $(e.currentTarget),
			value = select.prop('value');

		$('[data-sync-ref="'+select.data('sync-ref')+'"').prop('value', value);
	}

	$(document).on('change', 'select', synchronizeSelect);
});